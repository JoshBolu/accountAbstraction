...
contract MinimalAccountMultiSig is IAccount {
    ...
    error MinimalAccountMultiSig__NotFromEntryPointOrOwner();
        ...
        returns (uint256 validationData)
    {
        ...
        for (uint256 i = 0; i < sigCount; i++) {
            ...
            uint8 v;
            assembly {
                r := calldataload(add(userOp.signature.offset, offset))
                s := calldataload(add(userOp.signature.offset, add(offset, 32)))
                v := byte(0, calldataload(add(userOp.signature.offset, add(offset, 64))))
            }
    ...
    function _requireFromEntryPointOrOwner() internal view {
        if (msg.sender != address(I_ENTRY_POINT) && !isOwner[msg.sender]) {
            revert MinimalAccountMultiSig__NotFromEntryPointOrOwner();
        }
    }
